...
async def _payload(
    api_client: httpx.AsyncClient, run_id: str, **values: object
) -> dict[str, object]:
    run = await api_client.get(f"/api/runs/{run_id}")
    assert run.status_code == 200, run.text
    return {"expected_review_revision": run.json()["review_revision"], **values}


@pytest.mark.asyncio(loop_scope="session")
async def test_review_revision_is_mandatory(
    api_client: httpx.AsyncClient, reconciled_run: dict[str, str]
) -> None:
    response = await api_client.post(
        f"/api/runs/{reconciled_run['run_id']}/cases/CASE_FV0056/assign",
        json={"owner_role": "FINANCE_OPERATIONS"},
        headers=_headers("missing-revision"),
    )
...
async def test_valid_approval_passes_invariants(
    ...
    response = await api_client.post(
        f"/api/runs/{reconciled_run['run_id']}/cases/CASE_0001/approve",
        json=await _payload(
            api_client,
            reconciled_run["run_id"],
...
async def test_invariant_failed_approval_moves_to_pending_verification(
    ...
    response = await api_client.post(
        f"/api/runs/{reconciled_run['run_id']}/cases/CASE_FV0056/approve",
        json=await _payload(
            api_client,
            reconciled_run["run_id"],
            actor="reviewer@example.test",
    ...
    detail = await api_client.get(f"/api/runs/{reconciled_run['run_id']}/cases/CASE_FV0056")
...
async def test_reject_defer_assign_and_task(
    ...
    rejected = await api_client.post(
        f"/api/runs/{reconciled_run['run_id']}/cases/CASE_FV0057/reject",
        json=await _payload(
            api_client,
            reconciled_run["run_id"],
            actor="reviewer@example.test",
    ...
    deferred = await api_client.post(
        f"/api/runs/{reconciled_run['run_id']}/cases/CASE_MS0070/defer",
        json=await _payload(
            api_client,
            reconciled_run["run_id"],
            actor="reviewer@example.test",
    ...
    assigned = await api_client.post(
        f"/api/runs/{reconciled_run['run_id']}/cases/CASE_FV0058/assign",
        json=await _payload(
            api_client,
            reconciled_run["run_id"],
            actor="lead@example.test",
            owner_role="FINANCE_OPERATIONS",
        ),
    ...
    task = await api_client.post(
        f"/api/runs/{reconciled_run['run_id']}/cases/CASE_FV0058/tasks",
        json=await _payload(
            api_client,
            reconciled_run["run_id"],
            actor="lead@example.test",
...
async def test_reconciled_case_cannot_be_deferred(
    ...
    response = await api_client.post(
        f"/api/runs/{reconciled_run['run_id']}/cases/CASE_0001/defer",
        json=await _payload(
            api_client,
            reconciled_run["run_id"],
